...
class Model_Table extends CI_Model
{
    ...
    public function getBankTransactionSummary($accountId = null, $date = null)
    {
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select ifnull(sum(sm.SaleMaster_PaidAmount), 0) from tbl_salesmaster sm
                    where sm.Bank_Account = ba.account_id
                    and sm.Status = 'a'
                ...
                (
                    select ifnull(sum(pm.PurchaseMaster_PaidAmount), 0) from tbl_purchasemaster pm
                    where pm.bank_name = ba.account_id
                    and pm.status = 'a'
                    and pm.payment_method = 'Bank'
                    and pm.PurchaseMaster_BranchID = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and pm.PurchaseMaster_OrderDate < '$date'") . "
                ) as total_payment_to_supplier,
                (
                    select (ba.initial_balance + total_deposit + total_interest + total_received_from_customer + total_received_from_supplier + total_sales_received_from_customer) - (total_withdraw + total_charge + total_paid_to_customer + total_paid_to_supplier + total_payment_to_supplier)
                ) as balance
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(sm.SaleMaster_PaidAmount + sm.SaleMaster_BankPaidAmount), 0.00)
                from tbl_salesmaster sm
                where sm.SalseCustomer_IDNo = c.Customer_SlNo
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
            ...
            (select ifnull(sum(cp.CPayment_discount), 0.00) 
                ...
                and cp.CPayment_TransactionType = 'CR'
                " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                and cp.CPayment_status = 'a') as cashDiscountCR,
            ...
            (select ifnull(sum(cp.CPayment_discount), 0.00) 
                ...
                and cp.CPayment_TransactionType = 'CP'
                " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                and cp.CPayment_status = 'a') as paidDiscountCP,
            ...
            (select (billAmount + paidOutAmount) - (paidAmount + cashDiscountCR + returnedAmount)) as dueAmount
