...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            ...
            $customerPaymentClause = " and cp.CPayment_date between '$data->dateFrom' and '$data->dateTo'";
    ...
    public function getBankTransactions()
    {
        ...
        if (isset($data->transactionTypeCharge) && $data->transactionTypeCharge == 'charge') {
            $typeClause .= " or bt.transaction_type = '$data->transactionTypeCharge'";
        }

        if (isset($data->transactionTypeInterest) && $data->transactionTypeInterest == 'interest') {
            $typeClause .= " or bt.transaction_type = '$data->transactionTypeInterest'";
        }
    ...
    public function getAllBankTransactions()
    {
        ...
        $transactions = $this->db->query("
            select * from(
                ...
                join tbl_bank_accounts ac on ac.account_id = sm.Bank_Account
                join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
                where sm.Bank_Account is not null
                and sm.Status = 'a'
                ...
                select
                    ...
                    0.00 as balance
                from tbl_purchasemaster pm
                join tbl_bank_accounts ac on ac.account_id = pm.bank_name
                join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
                where pm.bank_name is not null
                and pm.Status = 'a'
